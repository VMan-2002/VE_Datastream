...
import ScriptingDatastream.ScriptingDatastreamCpp;
...
class DatastreamFuncs {
    public static var screenResolution:FlxPoint;

	public static function exec(type:String) {
		//todo: implement this
		//todo: restrict this
	}
	
	public static function screenCapture(targetName:String, rtGraphic:Bool)/*:Null<BitmapData>*/ {
        ScriptingDatastreamCpp.screenshotFilePath = Main.execPath + 'assets/data/content/${targetName}.png';
        ScriptingDatastreamCpp.screenshotFilePathLength = ScriptingDatastreamCpp.screenshotFilePath.length;
        ScriptingDatastreamCpp.screenCapture("", Math.floor(screenResolution.x), Math.floor(screenResolution.y));
		/*if (rtGraphic)
            return getContentPNG(targetName);
        return null;*/
	}

    public static function getContentPNG(name:String) {
        return BitmapData.fromFile('assets/data/content/${name}.png');
    }
    ...
    public static function setBorderless(n:Bool) {
        getWindow().borderless = n;
    }

    public static var transparent:Bool = false;

    public static function setTransWindow(state:Bool, n:Int) {
        if (state == transparent)
            return;
        if (state)
            FlxTransWindow.getWindowsTransparent(n);
        else
            FlxTransWindow.getWindowsbackward(n);
        transparent = state;
    }

    private static final rStopPath = "assets/data/content/rstop.bat";

    public static function setRStop(n:Bool) {
        if (n)
            File.saveContent(rStopPath, "del rstop.bat");
        else if (getRStop())
            FileSystem.deleteFile(rStopPath);
    }

    public static function getRStop() {
        return FileSystem.exists(rStopPath);
    }

    public static function screenGlitchShader() {
        return new ScreenGlitchShader();
    }

    public static function outlineShader() {
        return new OutlineShader();
    }

    public static function scaleMode(type:String) {
        FlxG.scaleMode = switch(type.toLowerCase()) {
            case "noletterbox":
                new NoLetterboxScaleMode();
            case "ratiofill":
                new RatioScaleMode(true);
            default:
                new RatioScaleMode(false);
        };
    }
}

class ScreenGlitchShader extends FlxShader {
	@:glFragmentSource('
        #pragma header
        uniform float vert;
        uniform float o;
        ...
        void main() {
            ...
            vec2 uvr = openfl_TextureCoordv;
            vec4 outCol = flixel_texture2D(bitmap, uvr);
            
            if (uvr.y > vert) {
                vec2 uvg = openfl_TextureCoordv;
                vec2 uvb = openfl_TextureCoordv;
                
                //float bprob = -0.3 + (iMouse.x / 500.0);
                float br = mod(((floor(uvr.x * 16.0 * aspect) * 6161.3) - ((floor((uvr.y + mod(uvr.x * 0.5 * aspect, 0.1)) * 9.0)) * 4417.3) + (floor(uvr.x * 4.0 * aspect) * 2372.3)) / 3.0, 1.0);

                if (bprob > br) {
                    outCol = vec4(br / 6.0, br / 5.0, br / 4.0, 1.0);
                } else {
                    float tear = pow(abs(mod(uvr.y * 16, 2.0) - 1.0), 3.0) * tearamount;
                    xm = (xm + tear) * aspect;
                    if (mod(uvr.y + o, 0.07) > mod(uvr.y, 0.1)) {
                        uvr.x -= xm;
                    }
                    if (mod(uvr.y - o, 0.09) > mod(uvr.y, 0.1)) {
                        uvg.x -= xm;
                    }
                    if (mod(uvr.y - o, 0.04) < mod(uvr.y, 0.1)) {
                        uvb.x -= xm;
                    }

                    // Output to screen
                    outCol = vec4(flixel_texture2D(bitmap, uvr).r, flixel_texture2D(bitmap, uvg).g, flixel_texture2D(bitmap, uvb).b, (flixel_texture2D(bitmap, uvr).a + flixel_texture2D(bitmap, uvg).a + flixel_texture2D(bitmap, uvb).a) / 1.0);
                }
            }
			gl_FragColor = outCol;
        }
	...
	public function new()
	{
		...
		xm.value = [0.0];
		bprob.value = [-0.01];
		tearamount.value = [0.01];
        aspect.value = [1.0];
        setGameAspect();
	}

    public function setAspect(w:Int, h:Int) {
        aspect.value[0] = (w / h) / (16 / 9);
    }

    public function setGameAspect()
        setAspect(FlxG.width, FlxG.height);

    public function setScreenAspect()
        setAspect(Math.floor(DatastreamFuncs.screenResolution.x), Math.floor(DatastreamFuncs.screenResolution.y));
}
...
class OutlineShader extends FlxShader {
	@:glFragmentSource('
        ...
        void main()
        {
            ...
            if (col.a < 0.01) {
                col.a = 0;
            } else if (col.r + col.g + col.b < threshold) {
                col.rgb = outline.rgb;
                //col.a *= outline.a;
            } else if (bfill) {
                col.rgb = fill.rgb;
	...
	public function new()
	{
        ...
        threshold.value = [0.1];
        bfill.value = [true];
        outline.value = [1.0, 1.0, 1.0, 1.0];
        fill.value = [0.0, 0.0, 0.0, 1.0];
	}

    public function setOutline(r:Null<Float>, g:Null<Float>, b:Null<Float>, a:Null<Float>) {
        if (r != null)
            outline.value[0] = r;
        if (g != null)
            outline.value[1] = g;
    ...
    public function setFill(r:Null<Float>, g:Null<Float>, b:Null<Float>, a:Null<Float>) {
        if (r != null)
            fill.value[0] = r;
        if (g != null)
            fill.value[1] = g;
