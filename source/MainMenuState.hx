...
class MainMenuState extends MusicBeatState {
	...
	override function create() {
		...
		var versionShit:FlxText = new FlxText(5, FlxG.height - 20, 0, "v" + Application.current.meta.get('version'), 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat(Main.codeFontBold, 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);

		var vmanEngineThing:FlxText = new FlxText(5, FlxG.height - 40, 0, '${Translation.getTranslation("vman engine", "game")} - ${Main.gameVersionStr}', 12);
		vmanEngineThing.scrollFactor.set();
		vmanEngineThing.setFormat(Main.codeFontBold, 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(vmanEngineThing);
