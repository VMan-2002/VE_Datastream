...
class FreeplayState extends MusicBeatState {
	...
	override function create() {
		...
		scoreText = new FlxText(1280, 5, FlxG.width, Translation.getTranslation("personal best", "freeplay", ["1234567890"]), 32);
		scoreText.x -= scoreText.textField.textWidth + 2;
		scoreText.fieldWidth -= scoreText.x;
		// scoreText.autoSize = false;
		scoreText.setFormat(Main.codeFont, 32, FlxColor.WHITE, LEFT);
		Translation.setObjectFont(scoreText, "vcr font");
		// scoreText.alignment = RIGHT;
		
		folderDirText.font = Main.codeFontBold;
		...
		scoreBG2.x = 1280 - scoreBG2.width;
		scoreBG2.alpha = 0.6;
	...
	public function makeSonglist(list:Array<SongMetadata>) {
		...
		folderDirText.text = (Options.selfAware ? Main.execPath : "C:\\Users\\User\\VE_Datastream\\").replace("\\", slash);
		/*if (isCornflower) {
			cornflowerClass.makeSonglist(this);
		}*/
	}
