...
class OptionsSubState extends OptionsSubStateBasic
{
	override function optionList() {
		...
		if (!Options.letUseModchart) {
			a.remove("Enable modcharts");
		}
		return a;
	}
	...
	override function optionDescription(name:String) {
		switch(name) {
			...
			case "self awareness":
				return ["You know what this does by now.", Options.selfAware ? "Enabled" : "Disabled"];
	...
	override function optionUpcoming(name:String) {
		switch (name) {
			case "master volume" | "sound volume" | "instrumental volume" | "vocals volume" | "input offset calibrate" | "save data management":
				return true;
		}
		return false;
	}
